...
const templates = {
    // Templates Reclame Aqui
    'ra-resposta-padrao': {
        titulo: 'Resposta Padrão - Reclame Aqui',
        conteudo: `Prezado(a) cliente,

Agradecemos seu contato e lamentamos pelo transtorno causado.

Nossa equipe está analisando sua solicitação e tomaremos as medidas necessárias para resolver a situação.

Seguimos à disposição para ajudar.`
    },
    'ra-resolvido': {
        titulo: 'Problema Resolvido - Reclame Aqui',
        conteudo: `Prezado(a) cliente,

Informamos que sua solicitação foi devidamente atendida e o problema foi resolvido.

Conforme já havíamos encaminhado anteriormente, todas as medidas necessárias foram implementadas para garantir a regularização da situação.

Seguimos à disposição para ajudar.`
    },
    'ra-em-analise': {
        titulo: 'Em Análise - Reclame Aqui',
        conteudo: `Prezado(a) cliente,

Sua solicitação está sendo analisada por nossa equipe especializada.

Em breve entraremos em contato com as informações necessárias para resolver sua situação.

Seguimos à disposição para ajudar.`
    },

    // Templates Explicações
    'explicacao-fgts': {
        titulo: 'Explicação sobre FGTS',
        conteudo: `Prezado(a) cliente,

Vamos esclarecer sobre o FGTS (Fundo de Garantia do Tempo de Serviço):

1. O que é o FGTS: É um fundo que garante proteção ao trabalhador demitido sem justa causa.

2. Como funciona: O empregador deposita mensalmente 8% do salário do funcionário em uma conta vinculada.

3. Restituições: Podem ocorrer em casos específicos como demissão sem justa causa, aposentadoria, ou compra da casa própria.

4. Consulta: Você pode verificar seus saldos no aplicativo FGTS ou no site da Caixa Econômica Federal.

Nossa equipe está disponível para orientações adicionais sobre seu caso específico.`
    },
    'explicacao-malha-fina': {
        titulo: 'Explicação sobre Malha Fina',
        conteudo: `Prezado(a) cliente,

Vamos esclarecer sobre a Malha Fina:

1. O que é: É um sistema de fiscalização da Receita Federal que identifica inconsistências na declaração do IR.

2. Como funciona: O sistema compara as informações declaradas com dados de terceiros.

3. Prazo para resposta: Você tem 30 dias para se manifestar após receber a notificação.

4. Como resolver: É necessário apresentar documentos que comprovem as informações declaradas.

5. Penalidades: Caso não seja respondida, pode gerar multas e juros.

Nossa equipe está disponível para orientações sobre como proceder em seu caso específico.`
    },
    'explicacao-ecac': {
        titulo: 'Procedimentos no eCAC',
        conteudo: `Prezado(a) cliente,

Vamos orientar sobre os procedimentos no eCAC (Centro Virtual de Atendimento):

1. Acesso: Acesse www.gov.br/receitafederal e clique em "eCAC"

2. Login: Use seu CPF e senha ou certifique digital

3. Navegação: No menu principal, localize a opção desejada

4. Documentos: Tenha em mãos os documentos necessários

5. Prazo: Fique atento aos prazos estabelecidos

Para revogação de procurações:
- Acesse "Procurações"
- Selecione "Revogar Procuração"
- Siga as instruções na tela

Nossa equipe está disponível para orientações adicionais.`
    },
    'explicacao-exclusao': {
        titulo: 'Exclusão de Cadastro',
        conteudo: `Prezado(a) cliente,

Para solicitar a exclusão de seu cadastro, siga os passos abaixo:

1. Envie um e-mail para nossa equipe solicitando a exclusão
2. Informe seu CPF e dados de identificação
3. Especifique o motivo da exclusão
4. Aguarde nossa confirmação por e-mail

A exclusão será realizada em até 15 dias úteis, conforme previsto na LGPD.

Nossa equipe está disponível para orientações adicionais.`
    },
    'explicacao-lgpd': {
        titulo: 'Questões de LGPD',
        conteudo: `Prezado(a) cliente,

Conforme a Lei Geral de Proteção de Dados (LGPD - Lei nº 13.709/2018), você tem os seguintes direitos:

1. Acesso: Solicitar informações sobre seus dados
2. Correção: Solicitar correção de dados incorretos
3. Exclusão: Solicitar a exclusão de seus dados
4. Portabilidade: Solicitar a transferência de seus dados

Para exercer qualquer um desses direitos, entre em contato conosco.

Nossa equipe está disponível para orientações sobre a LGPD.`
    },
    'explicacao-procuracoes': {
        titulo: 'Revogação de Procurações',
        conteudo: `Prezado(a) cliente,

Para revogar procurações no eCAC, siga os passos abaixo:

1. Acesse www.gov.br/receitafederal
2. Clique em "eCAC" e faça login
3. No menu, selecione "Procurações"
4. Escolha "Revogar Procuração"
5. Selecione a procuração a ser revogada
6. Confirme a operação

A revogação é imediata e você receberá confirmação por e-mail.

Em caso de dificuldades, nossa equipe está disponível para orientações.`
    },
    'explicacao-ccb': {
        titulo: 'Cédula de Crédito Bancário',
        conteudo: `Prezado(a) cliente,

A Cédula de Crédito Bancário (CCB) é um título de crédito que representa uma promessa de pagamento. Vamos esclarecer:

1. O que é: É um título de crédito emitido por instituições financeiras
2. Como funciona: Representa uma promessa de pagamento de valor determinado
3. Características: Pode ser negociada no mercado financeiro
4. Riscos: Está sujeita aos riscos do mercado
...
Nossa equipe está disponível para orientações adicionais.`
    ...
    // Templates E-mails
    'email-lgpd': {
        titulo: 'E-mail sobre LGPD',
        conteudo: `Prezado(a) [NOME],

Conforme solicitado, informamos que estamos processando sua solicitação de [TIPO_DE_SOLICITACAO] conforme previsto na Lei Geral de Proteção de Dados (LGPD - Lei nº 13.709/2018).

O prazo para resposta é de até 15 dias úteis, conforme estabelecido na legislação.
...
Equipe Velotax`
    ...
    'email-exclusao': {
        titulo: 'E-mail de Exclusão',
        conteudo: `Prezado(a) [NOME],

Confirmamos o recebimento de sua solicitação de exclusão de dados.

Conforme previsto na LGPD, sua solicitação será processada em até 15 dias úteis.
...
Equipe Velotax`
    ...
    'email-procuracoes': {
        titulo: 'E-mail sobre Procurações',
        conteudo: `Prezado(a) [NOME],

Informamos que sua solicitação de revogação de procuração foi processada com sucesso.

A revogação é imediata e você pode verificar no eCAC (www.gov.br/receitafederal).
...
async function gerarRespostaOpenAI() {
    ...
    try {
        ...
        const resposta = await fetch('/api/generate-response', {
            method: 'POST',
            headers: {
...
function carregarTemplate() {
    const categoria = document.getElementById('categoria-template').value;
    const templateEspecifico = document.getElementById('template-especifico').value;
    ...
    if (templateEspecifico) {
        template = templates[templateEspecifico];
    } else {
        // Carrega o primeiro template da categoria
        const templatesCategoria = Object.keys(templates).filter(key => key.startsWith(categoria.replace('-', '')));
        if (templatesCategoria.length > 0) {
            template = templates[templatesCategoria[0]];
        }
    }
    
    if (template) {
        document.getElementById('template-content').innerHTML = template.conteudo;
        document.getElementById('template-resultado').style.display = 'block';
        
        stats.templatesUsados++;
        updateStats();
        
        showSuccessMessage('Template carregado com sucesso!');
    } else {
        showErrorMessage('Template não encontrado.');
    }
}

// Atualizar lista de templates específicos baseado na categoria
document.getElementById('categoria-template').addEventListener('change', function() {
    const categoria = this.value;
    const templateEspecifico = document.getElementById('template-especifico');
    
    templateEspecifico.innerHTML = '<option value="">Selecione o template...</option>';
    
    if (categoria) {
        const templatesCategoria = Object.keys(templates).filter(key => key.startsWith(categoria.replace('-', '')));
        templatesCategoria.forEach(key => {
            const option = document.createElement('option');
            option.value = key;
            option.textContent = templates[key].titulo;
            templateEspecifico.appendChild(option);
        });
    }
});
...
function updateStats() {
    ...
    document.querySelectorAll('.stat-value')[2].textContent = stats.templatesUsados;
}
